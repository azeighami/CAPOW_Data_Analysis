...
column_name = ['Local Damages', 'Market Price', 'Racial Inequality','Pollution Burden','CAISO Deamnd', 'PNW Load','CAISO Hydropower','PNW Hydropower','Path66 Flow','CAISO Wind Power','PNW Wind Power', 'CAISO Solar Power']
cmap = plt.cm.RdBu_r
# cmap = 'coolwarm'
...
#     # plot_df = plot_df2 = pd.concat([SNP_damage_percentile_df.loc[:,k], Shadow_price_percentile_df.loc [:,k] , stochastic_percentile_df] , axis = 1)
#     # plot_df.columns = column_name
#     # plot_df2.columns = column_name
...
#         # if plot_df2.loc[i,'Local Damages'] == plot_df2['Local Damages'].min():
#         #     plot_df2.loc[i,'Dataset'] = 'Lowest'
            
#         # if plot_df2.loc[i,'Local Damages'] == plot_df2['Local Damages'].max():
...
#     # cbar = plt.colorbar(mappable=cbar, orientation='vertical', pad = 0.068, shrink= 0.98, ticks=[0.02,0.98])
#     # cbar.ax.set_yticklabels(ticks, fontsize=14)
#     # cbar.ax.set_ylabel ('Local Damages', labelpad= -30,fontsize=16 )
    
#     # plt.title('Parallel Plot of Emission Damage vs Power System Variables', fontsize=14)
#     plt.savefig('Plots/Fig_6_Parallel_{}'.format(k), bbox_inches='tight',dpi=250)
#     # plt.clf()
...
for k in scenarios:
    ...
    plot_df = plot_df2 = pd.concat([SNP_damage_df.loc[:,k] , Shadow_price_df.loc [:,k] , stochastic_df ] , axis = 1)
    column_name = ['Local Damages', 'Market Price','CAISO Demand', 'PNW Load','CAISO Hydropower','PNW Hydropower','Path66 Flow','CAISO Wind Power','PNW Wind Power', 'CAISO Solar Power', 'Year']
    # column_name = ['Local Damages', 'Market Price','CAISO Deamnd', 'PNW Load','CAISO Hydropower','PNW Hydropower','Path66 Flow','CAISO Wind Power','PNW Wind Power', 'CAISO Solar Power']
    plot_df.columns = column_name
    plot_df2.columns = column_name
    # , race[k],pollution_burden[k]
    ...
    plot_df = plot_df.drop(columns = ['PNW Load','Path66 Flow' , 'PNW Wind Power', "Year"])
    plot_df2 = plot_df2.drop(columns = ['PNW Load','Path66 Flow' , 'PNW Wind Power', "Year"])
    
    
    csfont = {'fontname':'arial'}
    heatmap = plot_df2.corr(method='pearson')
    Pval = plot_df2.corr(method=pearsonr_pval)
    mask = np.triu(np.ones_like(heatmap, dtype=bool))

    mask = np.delete(mask,6, axis = 1)
    ...
    plt.savefig('Plots/Fig_6_heatmap_Daily_{}'.format(k), bbox_inches='tight',dpi=250)
