...
scenarios = ['no_tax']# ,'SNP']# ,'CO2', 'all_tax']
directory = [ 1, 2, 3, 4, 5]
sim_years = 100
...
for s in scenarios:
    ...
    for i in directory:
        ...
        for j in range (sim_years):
            ...
            for gen in ['METEC_2_PL1X3', 'GATWAY_2_PL1X3', 'CROKET_7_UNIT', 'DELTA_2_PL1X4', 'LMEC_1_PL1X3']:# generators.index:
                # mwh_temp = pd.DataFrame(mwh_simp[gen])
                # mwh_temp['Day'] = Day//24
                # mwh_temp['hour'] = Day-mwh_temp['Day']*24
                # mwh_avg = mwh_temp.groupby(["Day"])['Value'].sum()
                production.loc[:, gen] = mwh_simp[gen]
...
production_df.to_csv (r'5years/production_df_all_tax_hourly.csv', index = False, header=True) 
